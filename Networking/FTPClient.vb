...
Public Class FTPclient
    ...
    Public Event DownloadProgress As EventHandler(Of FTPClientDownloadProgressEventArgs)
...
#Region "Download: File transfer FROM ftp server"
    ...
    Public Function Download(ByVal sourceFilename As String, ByVal localFilename As String, Optional ByVal PermitOverwrite As Boolean = False, Optional ByVal fileSize As Integer = -1) As Boolean
        '2. determine target file
        Dim fi As New FileInfo(localFilename)
        Return Me.Download(sourceFilename, fi, PermitOverwrite, fileSize)
    End Function

    'Version taking an FtpFileInfo
    Public Function Download(ByVal file As FTPfileInfo, ByVal localFilename As String, Optional ByVal PermitOverwrite As Boolean = False, Optional ByVal fileSize As Integer = -1) As Boolean
        Return Me.Download(file.FullName, localFilename, PermitOverwrite, file.Size)
    End Function

    'Another version taking FtpFileInfo and FileInfo
    Public Function Download(ByVal file As FTPfileInfo, ByVal localFI As FileInfo, Optional ByVal PermitOverwrite As Boolean = False, Optional ByVal fileSize As Integer = -1) As Boolean
        Return Me.Download(file.FullName, localFI, PermitOverwrite)
    End Function

    'Version taking string/FileInfo
    Public Function Download(ByVal sourceFilename As String, ByVal targetFI As FileInfo, Optional ByVal PermitOverwrite As Boolean = False, Optional ByVal fileSize As Integer = -1) As Boolean
        '1. check target
                ...
                Using fs As FileStream = targetFI.OpenWrite
                    Try
                        ...
                        Do
                            ...
                            If fileSize >= 0 Then
                                bytesRead += read
                                pct = Int((bytesRead / fileSize) * 100)
                                If pct > lastPct Then
                                    'raise event
                                    lastPct = pct
                                    RaiseEvent DownloadProgress(Me, New FTPClientDownloadProgressEventArgs(sourceFilename, pct))
                                End If
                            End If
                        Loop Until read = 0
...
#Region "Regular expressions for parsing LIST results"
    ...
    Private Shared _ParseFormats As String() = { _
        ...
        "(?<dir>[\-d])(?<permission>([\-r][\-w][\-xs]){3})(\s+)(?<size>(\d+))(\s+)(?<ctbit>([A-Za-z-]+\s+[A-Za-z-]+))(\s+)(?<size2>(\d+))\s+(?<timestamp>\w+\s+\d+\s+\d{2}:\d{2})\s+(?<name>.+)", _
        "(?<dir>[\-d])(?<permission>([\-r][\-w][\-xs]){3})\s+\d+\s+\d+\s+(?<size>\d+)\s+(?<timestamp>\w+\s+\d+\s+\d{4})\s+(?<name>.+)", _
        "(?<dir>[\-d])(?<permission>([\-r][\-w][\-xs]){3})\s+\d+\s+\-+\s+\-+\s+(?<size>(\d+))+\s+(?<timestamp>\w+\s+\d+\s+\d{4})\s+(?<name>.+)", _
    ...
    Private Function GetFileOrDir(ByVal type As FTPfileInfo.DirectoryEntryTypes, Optional ByVal ext As String = "") As FTPdirectory
        ...
        For Each fi As FTPfileInfo In Me
            If fi.FileType = type Then
                If ext = "" Then
                    result.Add(fi)
                ElseIf ext.ToUpper = fi.Extension.ToUpper Then
                    result.Add(fi)
                End If
            End If
        Next
